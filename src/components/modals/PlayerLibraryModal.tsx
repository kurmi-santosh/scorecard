...
export function PlayerLibraryModal({ visible, name, players, error, onClose, onNameChange, onSave }: Props) {
  return (
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <SafeAreaView style={styles.modalSafe}>
        <KeyboardAvoidingView style={styles.modalKeyboard} behavior={Platform.OS === "ios" ? "padding" : undefined}>
          <View style={styles.modalHeader}><Text style={styles.modalTitle}>Saved players</Text><Pressable onPress={onClose}><Text style={styles.closeText}>Done</Text></Pressable></View>
          <ScrollView contentContainerStyle={styles.modalContent} keyboardShouldPersistTaps="handled">
            ...
            <TextInput value={name} onChangeText={onNameChange} placeholder="e.g. Asha" placeholderTextColor="#7A8798" style={styles.nameInput} maxLength={24} returnKeyType="done" onSubmitEditing={onSave} />
            {!!error && <Text style={styles.errorText}>{error}</Text>}
            <PrimaryButton label="Save player" onPress={onSave} />
            <Text style={styles.formTitle}>Your players</Text>
