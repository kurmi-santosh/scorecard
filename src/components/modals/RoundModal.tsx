    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <SafeAreaView style={styles.modalSafe}>
        <KeyboardAvoidingView style={styles.modalKeyboard} behavior={Platform.OS === "ios" ? "padding" : undefined}>
          <View style={styles.modalHeader}><Text style={styles.modalTitle}>{roundToEdit ? "Edit" : "Round"} {roundNumber}</Text><Pressable onPress={onClose} accessibilityRole="button" accessibilityLabel={roundToEdit ? "Close editor" : "Cancel round"}><Text style={roundToEdit ? styles.dangerCloseText : styles.closeText}>{roundToEdit ? "Close" : "Cancel"}</Text></Pressable></View>
          <ScrollView contentContainerStyle={styles.modalContent} keyboardShouldPersistTaps="handled">
            {roundPlayers.map((player) => {
              const entry = draft[player.id] ?? { kind: null, score: "" };
              return (
                <View style={styles.roundCard} key={player.id}>
                  <View style={styles.roundPlayerHeader}><Text style={styles.roundPlayerName}>{player.name}</Text><Text style={styles.roundPlayerTotal}>{player.total} / {game.rules.maxScore}</Text></View>
                  ...
                  <NumberField value={entry.score} placeholder="Score" accessibilityLabel="Manual score" compact onChangeText={(value) => onChange(player.id, { kind: "manual", score: value })} />
                </View>
              );
            })}
