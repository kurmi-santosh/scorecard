...
import { MAX_PLAYER_NAME_LENGTH, MAX_PLAYERS } from "../../constants";
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <SafeAreaView style={styles.modalSafe}>
        <KeyboardAvoidingView style={styles.modalKeyboard} behavior={Platform.OS === "ios" ? "padding" : undefined}>
          <View style={styles.modalHeader}><Text style={styles.modalTitle}>Table Setup</Text><Pressable style={styles.modalCloseControl} onPress={onClose} accessibilityRole="button" accessibilityLabel="Close setup"><Text style={styles.dangerCloseText}>Close</Text></Pressable></View>
          <ScrollView contentContainerStyle={styles.modalContent} keyboardShouldPersistTaps="handled">
            <Text style={styles.modalLead}>Add players in order. Seats stay fixed.</Text>
            ...
            {names.map((name, index) => (
              <View style={styles.nameRow} key={index}>
                ...
                <TextInput value={name} onChangeText={(value) => onNameChange(index, value)} placeholder={`Player ${index + 1}`} placeholderTextColor="#7A8798" style={styles.nameInput} maxLength={MAX_PLAYER_NAME_LENGTH} returnKeyType="next" />
